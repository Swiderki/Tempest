...
export default class Player extends PhysicalGameObject {
  ...
  setPlayerPosition() {
    ...
    this.vertecies[0].x =
      this.game.level.vertecies[Math.floor(this.game.currentLevelSide)].x * 1.2 * (1 - levelShift) +
      this.game.level.vertecies[(Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints]
        .x *
        1.2 *
        levelShift;
    this.vertecies[0].y =
      this.game.level.vertecies[Math.floor(this.game.currentLevelSide)].y * 1.2 * (1 - levelShift) +
      this.game.level.vertecies[(Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints]
        .y *
        1.2 *
        levelShift;
    ...
    this.vertecies[1].x =
      this.game.level.vertecies[Math.floor(this.game.currentLevelSide)].x * 1.1 * (1 - levelShift) +
      this.game.level.vertecies[(Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints]
        .x *
        1.1 *
        levelShift;
    this.vertecies[1].y =
      this.game.level.vertecies[Math.floor(this.game.currentLevelSide)].y * 1.1 * (1 - levelShift) +
      this.game.level.vertecies[(Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints]
        .y *
        1.1 *
        levelShift;
    ...
    this.vertecies[3].x =
      this.game.level.vertecies[
        (Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints
      ].x;
    this.vertecies[3].y =
      this.game.level.vertecies[
        (Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints
    ...
    this.vertecies[4].x =
      (this.game.level.vertecies[Math.floor(this.game.currentLevelSide)].x * 0.7 +
        this.game.level.vertecies[
          (Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints
        ].x *
          0.3) *
      0.96;
    this.vertecies[4].y =
      (this.game.level.vertecies[Math.floor(this.game.currentLevelSide)].y * 0.7 +
        this.game.level.vertecies[
          (Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints
        ].y *
          0.3) *
    ...
    this.vertecies[5].x =
      (this.game.level.vertecies[Math.floor(this.game.currentLevelSide)].x * 0.3 +
        this.game.level.vertecies[
          (Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints
        ].x *
          0.7) *
      0.96;
    this.vertecies[5].y =
      (this.game.level.vertecies[Math.floor(this.game.currentLevelSide)].y * 0.3 +
        this.game.level.vertecies[
          (Math.floor(this.game.currentLevelSide) + 1) % this.game.level.numberOfPoints
        ].y *
          0.7) *
